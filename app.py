...
class BacktestEngine:
    ...
    def run_simulation(data, params):
        ...
        for i in range(len(data)):
            ...
            rolling_peak = max(price_history_x2) or 1
            ...
            if confirm_count >= confirm and pending_regime != current_regime:
                ...
                trades.append({'date': dates[i], 'from': old_regime, 'to': current_regime,
                               'label': label, 'val': total_val, 'cost': cost_impact})
                confirm_count = 0
        ...
        if not df_res.empty:
            ...
            # Add allocation weights for the area chart
            total_alloc = df_res['X1'] + df_res['X2']
            df_res['weight_x2'] = df_res['X2'] / total_alloc
            df_res['weight_x1'] = df_res['X1'] / total_alloc
...
def calculate_metrics(series):
    ...
    return {"Cumul": total_ret*100, "CAGR": cagr*100, "MaxDD": max_dd*100,
            "Vol": vol*100, "Sharpe": sharpe, "Calmar": calmar}
...
def get_data(tickers, start, end):
    ...
    for t in [x.strip().upper() for x in tickers]:
        try:
            ...
            if not df.empty:
                s = df.get('Close') or df.get('Adj Close') or df.iloc[:,0]
                price_map[t] = s
        except: continue
...
with col_main:
    ...
    if data.empty or len(data) < 10:
        ...
        # Ajouter les poids si inexistants pour l'UI
        df_res, trades = BacktestEngine.run_simulation(data, sim_params)
        if 'weight_x1' not in df_res.columns:
            df_res['weight_x1'] = 0
        if 'weight_x2' not in df_res.columns:
            df_res['weight_x2'] = 100 - df_res['weight_x1']
        ...
        with tabs[0]:
            ...
            fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.03,
                                row_heights=[0.7,0.3], subplot_titles=("Performance Comparée","Attribution d'Actifs"))

            # Performance lines
            fig.add_trace(go.Scatter(x=df_res.index, y=df_res['portfolio'], name='STRATÉGIE', line=dict(color='#A855F7', width=2)), row=1,col=1)
            fig.add_trace(go.Scatter(x=df_res.index, y=df_res['benchX2'], name='Risk (X2)', line=dict(color='#ef4444', width=1.5, dash='dot')), row=1,col=1)
            fig.add_trace(go.Scatter(x=df_res.index, y=df_res['benchX1'], name='Safe (X1)', line=dict(color='#10b981', width=1.5, dash='dot')), row=1,col=1)

            # Trade markers
            for t in trades:
                c = '#ef4444' if 'CRASH' in t['label'] else ('#f59e0b' if 'PRUDENCE' in t['label'] else '#10b981')
                symbol = "▼" if t['to'] != 0 else "▲"
                fig.add_annotation(x=t['date'], y=df_res.loc[t['date']]['portfolio'], text=symbol, showarrow=False, font=dict(color=c, size=14), row=1,col=1)

            # Allocation area
            fig.add_trace(go.Scatter(x=df_res.index, y=df_res['weight_x2'], name='Alloc X2', stackgroup='one', line=dict(width=0), fillcolor='rgba(239,68,68,0.5)'), row=2,col=1)
            fig.add_trace(go.Scatter(x=df_res.index, y=df_res['weight_x1'], name='Alloc X1', stackgroup='one', line=dict(width=0), fillcolor='rgba(16,185,129,0.5)'), row=2,col=1)
        ...
        with tabs[1]:
            ...
            fig_dd.update_layout(paper_bgcolor='#0A0A0F', plot_bgcolor='#0A0A0F', font=dict(family="Inter", color='#E0E0E0'), height=250, margin=dict(t=10,b=10))
        ...
        # ==========================================
        # TAB 4: VALIDATION (Monte Carlo)
        # ==========================================
        with tabs[3]:
            st.markdown("### 🛡️ Robustness Testing (Bootstrap)")
            ...
            if st.button("RUN MONTE CARLO (50 Runs)", key="monte_btn"):
        ...
        with tabs[4]:
            ...
            st.caption("Projection probabiliste du portefeuille sur 1 an basée sur volatilité récente.")
            if st.button("Générer les Scénarios (200 Simulations)", key="fan_btn"):
                with st.spinner("Calcul des trajectoires futures..."):
                    ...
                    fig_mc = go.Figure()
                    fig_mc.add_trace(go.Scatter(x=x_axis, y=p95_path, mode='lines', line=dict(width=0), showlegend=False, hoverinfo='skip'))
                    fig_mc.add_trace(go.Scatter(x=x_axis, y=p05_path, mode='lines', line=dict(width=0), fill='tonexty', fillcolor='rgba(168,85,247,0.1)', name='Intervalle 95%'))
                    fig_mc.add_trace(go.Scatter(x=x_axis, y=p75_path, mode='lines', line=dict(width=0), showlegend=False, hoverinfo='skip'))
                    fig_mc.add_trace(go.Scatter(x=x_axis, y=p25_path, mode='lines', line=dict(width=0), fill='tonexty', fillcolor='rgba(168,85,247,0.2)', name='Intervalle 50%'))
                    fig_mc.add_trace(go.Scatter(x=x_axis, y=median_path, mode='lines', name='Trajectoire Médiane', line=dict(color='#A855F7', width=3)))
                    fig_mc.add_hline(y=last_price, line_dash="dot", line_color="white", annotation_text="Aujourd'hui")
                    fig_mc.update_layout(paper_bgcolor='#0A0A0F', plot_bgcolor='#0A0A0F', font=dict(family="Inter", color='#E0E0E0'), height=500, title="Projection Future (Cône d'Incertitude)", xaxis_title="Jours Ouvrés (Futur)", yaxis_title="Valeur Portefeuille")
                    st.plotly_chart(fig_mc, use_container_width=True)
                    st.info("ℹ️ 200 futurs possibles basés sur la volatilité récente. Zone sombre = 50% des scénarios probables.")
