...
# ==========================================
# 2. ANALYTICS & MONTE CARLO (AVANCÉ)
# ==========================================
class AnalyticsEngine:
    ...
    def monte_carlo_forecast(series, n_sims=200, horizon=252):
        """
        Simule des trajectoires futures (Fan Chart) basé sur la volatilité historique récente.
        """
        returns = series.pct_change().dropna()
        # On utilise les 2 dernières années pour la dynamique récente
        ...
        for i in range(n_sims):
            # Bootstrap aléatoire des rendements historiques
...
def get_data(tickers, start, end):
    ...
    for t in [x.strip().upper() for x in tickers]:
        try:
            df = yf.download(t, start=start, end=end, progress=False, auto_adjust=True, threads=False)
            if df.empty: df = yf.download(t, start=start, end=end, progress=False, auto_adjust=False, threads=False)
    ...
    if data.empty or len(data) < 10:
        ...
        with tabs[1]:
            ...
            p_data = {
                ...
                "Benchmark": [f"{met_x2['CAGR']:.1f}%", f"{met_x2['Vol']:.1f}%", f"{met_x2['Sharpe']:.2f}", f"{met_x2['MaxDD']:.1f}%", f"{met_x2['Calmar']:.2f}", f"{met_x2['Cumul']:.1f}%"]
            }
