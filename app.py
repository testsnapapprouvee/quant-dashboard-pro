...
# ==========================================
# 1. CORE ENGINE (CORRIGÉ)
# ==========================================
class BacktestEngine:
    @staticmethod
    def run_simulation(data, params):
        cash_x2, cash_x1, portfolio = 100.0, 0.0, 100.0
        bench_x2, bench_x1 = 100.0, 100.0  # 🔧 FIX: Benchmarks normalisés
        ...
        for i in range(len(data)):
            ...
            if i > 0:
                ...
                # 🔧 FIX: Update benchmarks normalisés
                bench_x2 *= (1 + r_x2)
                bench_x1 *= (1 + r_x1)
            ...
            results.append({
                'date': dates[i], 
                'portfolio': portfolio, 
                'benchX2': bench_x2,  # 🔧 FIX: Déjà normalisé
                'benchX1': bench_x1,  # 🔧 FIX: Déjà normalisé
        ...
        # 🔧 FIX: Plus besoin de normaliser, déjà fait!
...
# ==========================================
# 3. MONTE CARLO FORECAST (CORRIGÉ)
# ==========================================
class MonteCarloForecaster:
    ...
    def run_forecast(data, params, n_simulations=500, forecast_days=252):
        ...
        try:
            ...
            # 🔧 FIX: Check if we have enough simulations
...
<div class="header-container">
    <div style="display:flex; justify-content:space-between; align-items:center;">
        <div>
            ...
            <p style="color:#888; margin:5px 0 0 0; font-size:12px;">ENGINE V2.1 • SILENT LUXURY • BUGS FIXED</p>
        </div>
...
with col_main:
    ...
    if data.empty or len(data) < 10:
        ...
        if df_res.empty:
            ...
            # --- TAB 1: DASHBOARD WITH DUAL CHART (CORRIGÉ) ---
            with tabs[0]:
                ...
                # 🔧 FIX: Create dual subplot with corrected data
                fig = make_subplots(
                    rows=2, cols=1,
                    row_heights=[0.7, 0.3],
                ...
                # BOTTOM: Allocation Area Chart (🔧 FIX: Corrected stacking)
                fig.add_trace(go.Scatter(
                    x=df_res.index,
                    y=df_res['alloc_X2'],
                    name='X2 (Risk)',
                ...
                if st.button("RUN MONTE CARLO (50 Runs)"):
                    with st.spinner("Simulating..."):
                        ...
                        if not mc_df.empty:
                            ...
                            st.error("Monte Carlo validation failed")

            # --- TAB 5: MONTE CARLO FORECAST (CORRIGÉ) ---
            with tabs[4]:
                ...
                <div class="legend-box">
                    ...
                    <p style="margin-top:8px; font-size:12px;">
                    This fan chart projects your portfolio 252 trading days (1 year) into the future using simulated scenarios.
                    ...
                    with st.spinner(f"Running {n_sims} Monte Carlo simulations..."):
                        ...
                        if not forecast.empty:
                            ...
                            fig_fan.add_trace(go.Scatter(
                                ...
                                mode='lines', line=dict(width=0),
                                fillcolor='rgba(168, 85, 247, 0.1)',
                                fill='tonexty', name='90% CI',
                                hovertemplate='5th-95th percentile<br>%{y:.1f}<extra></extra>'
                            ))
                            
                            fig_fan.add_trace(go.Scatter(
                                x=forecast.index, y=forecast['p75'],
                                mode='lines', line=dict(width=0),
                                showlegend=False, hoverinfo='skip'
                            ))
                            
                            fig_fan.add_trace(go.Scatter(
                                x=forecast.index, y=forecast['p25'],
                                mode='lines', line=dict(width=0),
                                fillcolor='rgba(168, 85, 247, 0.2)',
                                fill='tonexty', name='50% CI',
                            ...
                            fig_fan.update_layout(
                                ...
                                xaxis=dict(title="Date", showgrid=False, linecolor='#333'),
                                yaxis=dict(title="Portfolio Value", showgrid=True, gridcolor='rgba(255,255,255,0.05)'),
                                hovermode='x unified',
                            ...
                            st.error("Failed to generate forecast. Try adjusting parameters or check data quality.")
